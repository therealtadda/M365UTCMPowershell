...
Describe "Get-UTCMTenantDriftReport (orchestrator) with bind-time validation" {
    BeforeAll {
        ...
        Mock -ModuleName UTCM.Tools Get-UTCMAvailableSnapshot {
            @(@{ id='aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'; displayName='Snap'; createdDateTime='2026-02-08T10:00:00Z'; status='completed'})
        }
        Mock -ModuleName UTCM.Tools New-UTCMSnapshot { 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee' }
        Mock -ModuleName UTCM.Tools Get-UTCMSnapshot { @{ configurationItems = @(@{id='1'; displayName='A'; type='x'; data=@{x=1}}) } }
        Mock -ModuleName UTCM.Tools Get-UTCMCurrentStateSnapshot {
            @{ id='11111111-1111-1111-1111-111111111111'; configurationItems = @(@{id='2'; displayName='B'; type='x'; data=@{x=2}}) }
        }
